...
def analysis_var(code):
    code = rc.remove_comment(code)
...
# 变量代换主函数
def sub_var_main(code_1,code_2,filter = [1,1.5,1.5]):
    code1,var_in_code1 = analysis_var(code_1)
    code2,var_in_code2 = analysis_var(code_2)
...
def format_codeMain(code_1:str,code_2:str,subVar=True,filter = [1,1.5,1.5]):
    if subVar:
        code1,code2 = sub_var_main(code_1,code_2,filter=filter)
    else:
        code1,code2=rc.format_code_main(code_1),rc.format_code_main(code_2)
    return code1,code2
...
"""
'''
code1_path = "./test_code/测试题/A1.c++.txt"
code2_path = "./test_code/测试题/B1.c++.txt"
with open(code1_path, 'r') as test:
    code1 = test.read();
with open(code2_path, 'r') as test1:
    code2 = test1.read();

print(sub_var_main(code1,code2,[1,1.5,1.5]))
'''